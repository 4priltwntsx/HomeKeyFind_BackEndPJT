...
public class HouseDealDao {
	...
	public List<HouseDeal> selectAllByAptCode(long aptCode, String year, String month) throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				int idx = 1;
				HouseDeal h = new HouseDeal(rs.getLong(idx++),rs.getString(idx++), rs.getInt(idx++), rs.getInt(idx++), rs.getInt(idx++), rs.getString(idx++),rs.getString(idx++),rs.getString(idx++),rs.getLong(idx++));
				result.add(h);
			}
